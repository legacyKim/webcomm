...
export function middleware(req: NextRequest) {
    ...
    if (!token) {
        return NextResponse.json({ success: false, message: "토근이 만료되었습니다. 다시 로그인 해주세요." }, { status: 401 });
    }
    ...
    try {
        ...
        return NextResponse.json({ authenticated: true, username: username, userId: userId, userNick: userNick, userProfile: userProfile, userEmail: userEmail });
