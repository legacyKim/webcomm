...
export async function POST(req) {
  ...
  try {
    ...
    const user = await client.query("SELECT EXISTS (SELECT 1 FROM members WHERE username = $1)", [userid]);

    const nickname = await client.query("SELECT EXISTS (SELECT 1 FROM members WHERE user_nickname = $1)", [
      userNickname,
    ]);
