...
export async function POST(req) {
  ...
  try {
    ...
    if (existing.rowCount > 0) {
      ...
      return NextResponse.json({ success: false, message: "이미 신고한 게시글입니다." }, { status: 200 });
    }

    await client.query(
      `INSERT INTO post_actions (user_id, post_id, action_type, reason) VALUES ($1, $2, 'report', $3)`,
      [isUserId, postId, reason],
    );

    // posts 테이블의 reports 수 증가
    await client.query(`UPDATE posts SET reports = reports + 1 WHERE id = $1`, [postId]);
