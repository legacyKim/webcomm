...
export default function BoardManagePopup({ setPopupOpen, boardData, popupCorr, popupOption }
    : {
    ...
    const postBoard = async ({ boardName, urlSlug }: { boardName: string; urlSlug: string; }) => {
        ...
        if (boardData?.boards?.rows?.some(row => row.board_name === boardName)) {
            alert("게시판 명이 중복됩니다.");
            return;
        }
        ...
        if (boardData?.boards?.rows?.some(row => row.url_slug === urlSlug)) {
            alert("URL 명이 중복됩니다.");
            return;
        }
    ...
    const putBoard = async ({ id, seq, boardName, urlSlug }: { id: number; seq: number, boardName: string; urlSlug: string; }) => {

        if (boardData?.boards?.rows?.some(row => row.seq === seq)) {
            alert("순번이 중복됩니다.");
            return;
        }
            ...
            <div className="admin_popup admin_popup_mo">
                ...
                <div className="admin_popup_content">
                    <div className="input_box">
                        ...
                        <input type="text" value={seq} onChange={(e) => {
                    ...
                    <div className="input_box">
                        ...
                        <input type="text" value={boardName} onChange={(e) => {
                    ...
                    <div className='input_box'>
                        ...
                        <input type="text" value={urlSlug} onChange={(e) => {
                ...
                <div className="admin_popup_footer">
                    {popupOption === 'save' ? (
                        <button onClick={() => { postBoard({ boardName, urlSlug }); }}>저장</button>
                    ) : (
                        <button onClick={() => { id && seq && putBoard({ id, seq, boardName, urlSlug }); }}>저장</button>
                    )}
                </div>
            </div>
        </div>
